...
import { getPeriodoAnalisisElemento } from "../../pages/analisis/analisisQueryAPI";
import { usePutPeriodoAnalsisElemento } from "../../pages/analisis/analisisQueryAPI";

export const FormPutPeriodoAnalisis = ({ recordKey }) => {
    ...
    const { data: analisis, isLoading: loadingAnalisis, isError: errorAnalisis } = useQuery({
        queryKey: ['analisis_periodo', recordKey],
        queryFn: () => getPeriodoAnalisisElemento(recordKey),
        onSuccess: (analisis) => (
            reset({
                analisis: analisis.analisis.id,
                tipo_elemento: analisis.tipo_elemento.id,
                periodo: analisis.periodo.id
            })
        )
    })

    const { mutate: put, isLoading: putLoading, isError: putError } = usePutPeriodoAnalsisElemento()
            ...
            >
                <Controller
                    ...
                    render={({ field }) => (
                        <Select
                            {...field}
                            value={analisis?.analisis?.nombre || ''}
